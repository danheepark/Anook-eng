...
- **CONTEXT RESET RULE (STRICT)**: If the last AI message in `[과거 대화 맥락]` indicates that a previous request was already COMPLETED, ANSWERED, CONFIRMED, or ESCALATED (e.g., completion confirmation, handoff to staff, or closure), you MUST treat the user's current message as a completely NEW and INDEPENDENT request. DO NOT let the previous department's context or the escalation state bias your domain routing. Evaluate the new message from scratch.
...
- **OFFER REJECTION RULE**: 
  ...
  - **Case 2 (Task Confirmation/Details - IN-PROGRESS)**: If the AI is still asking for missing details (e.g., "어느 꽃집에서 배달해 드릴까요?") or asking for final confirmation (e.g., "Shall I process the delivery?"), and the user says "No", "안할게요", or "취소":
    - **CRITICAL OVERRIDE**: If there are items in `[고객의 현재 활성 요청(주문) 목록]`, it means the system has ALREADY created a PENDING ticket in the database. In this case, you MUST classify it as "CANCEL" to properly remove the PENDING ticket. If the user says "다 취소해줘" or "전부 취소", set domain to `null` and `cancel_scope` to `"ALL"` for bulk cancellation.
...
- **ESCALATION CONFIRMATION RULE**: If the last AI message in `[과거 대화 맥락]` asked if the guest wants to connect to the front desk or needs intervention (e.g., "프론트로 연결해 드릴까요?", "프론트 데스크의 직접적인 조치나 확인이 필요하신 상황일까요?", "connect you to the front desk"), and the guest agrees (e.g., "네", "조치해줘", "yes", "응"), you MUST classify it as "FRONT_ESCALATION" with domain "FRONT" and explicitly set `create_ticket: true`. IMPORTANT: If the escalation is specifically because the user wants more information (e.g., the AI asked "자세한 정보가 필요하시면 프론트로 연결해 드릴까요?"), you MUST include the exact text "INFO_ESCALATION" in the `reasoning` field.
  ...
  **EXCEPTION 1**: If the guest's current message contains a CLEAR NEW service request UNRELATED to the escalation offer, you MUST IGNORE the previous escalation offer and treat the message as a COMPLETELY NEW REQUEST according to the CONTEXT RESET RULE.
...
- **HYPERBOLIC COMPLIMENT RULE**: If the user uses extreme hyperbolic expressions in a clearly POSITIVE context (e.g., praising the view, food, or service), you MUST recognize it as a hyperbole/joke and classify it as "VOC" (POSITIVE). **HOWEVER (CRITICAL EXCEPTION)**: If the user explicitly describes a literal physical injury, trapped body parts, fire, or tangible danger, this OVERRIDES the joke rule. You MUST treat it as a real "EMERGENCY" (FRONT_ESCALATION) even if it starts with a compliment.
